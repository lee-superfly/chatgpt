...
public class UserController {
	...
	public Response findPassword(@ApiParam(value = "用户邮箱", required = true) @RequestParam String email) {
		Response response = new Response();
		...
		if (user != null) {
			...
			response.failure("该邮箱未注册！");
		}
		return response;
	}
	
	@ApiOperation(value = "发送链接到邮箱")
	@RequestMapping(value = "/sendPassLink", method = RequestMethod.POST, produces = "application/json; charset=UTF-8")
	public Response sendPassLink(@ApiParam(value = "用户邮箱", required = true) @RequestParam String email) {
		Response response = new Response();
		User user = userService.selectByEmail(email);
		if (user != null) {
			boolean result = userService.sendPassLink(email);
			if (result) {
				response.success();
			} else {
				response.failure();
	...
	@RequestMapping(value = "/restPass", method = RequestMethod.POST, consumes = "application/x-www-form-urlencoded", produces = "application/json; charset=UTF-8")
	public Response restPass(
			@ApiParam(value = "邮箱密文", required = true) @RequestParam("ec") String emailcipher,
			@ApiParam(value = "密码", required = true) @RequestParam("password") String password
	) {
		Response response = new Response();
		boolean result = userService.restPass(emailcipher, password);
		if (result) {
			response.success();
		} else {
			response.failure();
	...
	public Response selectPreNumById(
			@ApiParam(value = "机构代码", required = true) @RequestParam(value = "sqeNum") Integer id) {
		Response response = new Response();
